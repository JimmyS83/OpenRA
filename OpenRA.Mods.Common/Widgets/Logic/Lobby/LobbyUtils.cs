...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public static class LobbyUtils
	{
			...
			Session.Client c, OrderManager orderManager, Widget lobby, Action before, Action after)
		{
			...
			close = ShowRootPanel(panel, panelX, panelY);
		}

		/// <summary>
		/// Attaches a pre-built widget as a floating popup on <see cref="Ui.Root"/>, covering it with
		/// a transparent <see cref="MaskWidget"/> that dismisses the panel on any click outside it.
		/// Returns an action that removes both the panel and the mask when invoked.
		/// </summary>
		internal static Action ShowRootPanel(Widget panel, int x, int y)
		{
			...
			WorldRenderer worldRenderer, bool allowAdminLogin = false)
		{
			...
			name.OnRightClick = allowAdminLogin ? () => OpenAdminAuthPanel(orderManager) : null;
		...
		/// <summary>
		/// Opens the admin authentication panel, passing <paramref name="orderManager"/> so the
		/// submitted password can be dispatched as an order.
		/// </summary>
		public static void OpenAdminAuthPanel(OrderManager orderManager)
		{
			var wa = new WidgetArgs { { "orderManager", orderManager } };
			wa["args"] = wa;
			...
			WorldRenderer worldRenderer, Widget lobby, Action before, Action after)
		{
			...
			slot.IsVisible = () => orderManager.LocalClient.IsAdmin && c.Index != orderManager.LocalClient.Index;
			slot.IsDisabled = () => orderManager.LocalClient.IsReady;
